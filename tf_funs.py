...
def dynamics_linearized(X,model,xdim):
    ...
    #conversion takes dX/dt = Ax --> X_{t+1} = (I + A)X_t
...
def covariancePredict(x0,PI0,nsamps,T,gradfn,Q):
    #use a super-simplified particle filter-type approach to predict covariance 
    #T steps ahead in the future
    
    #x0: initial distribution mean
    #PI0: initial distribution covariance
    #nsamps: number of "parcticles" used to sample the distribution
    #gradfn: dynamics function, taking state as input
    xdim = len(x0)
    x_samps = np.zeros((nsamps,xdim,T))
    x_samps[:,:,0] = np.random.multivariate_normal(x0, PI0, (nsamps))
    
    #shape (nsamps, T, xdim)
    noisesamps = np.random.multivariate_normal(np.zeros((xdim)), Q, (nsamps,T))

    for k in range(1,T):
        for m in range(nsamps):
            grad_cont = gradfn(x_samps[m,:,k-1])        
            x_samps[m,:,k] = x_samps[m,:,k-1] + grad_cont + noisesamps[m,k,:]

    cov_particle = np.cov(np.transpose(x_samps[:,:,-1]))
    return cov_particle

def myopicController(X_est,PI_est,Control,gamma,true_model_est,true_model_est_null,target_model_est,xdim,udim):
    #the myopic controller graph
...
def myopicController_meanonly(X_est,PI_est,Control,gamma,true_model_est,
    ...
    #the first-order myopic control
...
def myopicController_noBdiff(X_est,PI_est,Control,gamma,true_model_est,
                              true_model_est_null,target_model_est,xdim,udim):
    #myopic controller, but for cases in which Jacobian B = dF/dx = 0. 
    #currently coded up because tensorflow doesn't play well with null return types
...
def lossPredict(x0,PI0,nsamps,gradfn,gradfn_targ,c):
    #easy sampling method to predict loss
    
    #x0: initial distribution mean
    #PI0: initial distribution covariance
    #nsamps: number of "parcticles" used to sample the distribution
    #gradfn: dynamcis function, taking state and control as input
    #gradfn_targ: dynamcis function, taking state as input
    
    xdim = len(x0)
    x_samps = np.random.multivariate_normal(x0, PI0, (nsamps))
    
    loss = 0

    for m in range(nsamps):
        grad_cont = gradfn(x_samps[m,:],c)  
        grad_targ = gradfn_targ(x_samps[m,:])
        loss += np.linalg.norm(grad_cont-grad_targ)**2
